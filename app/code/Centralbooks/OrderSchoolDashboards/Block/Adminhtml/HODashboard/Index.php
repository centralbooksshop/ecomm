...
class Index extends \Magento\Backend\Block\Template
{
    ...
    {    $starttime = microtime(true);
        if (!$this->overallOrders) {
			...
			$school_id = $objectManager->get('Magento\Backend\Model\Auth\Session')->getUser()->getSchoolwise();
            $connection = $this->resourceConnection->getConnection();
            $this->overallOrders = $connection->fetchAll("SELECT so.entity_id, so.school_id, so.increment_id, so.status, so.payment_status, so.created_at, so.shipping_method, sop.method from sales_order as so join sales_order_payment as sop on so.entity_id = sop.parent_id where created_at >= '$highDate' AND so.school_id = '$school_id' AND (`status` NOT IN('order_split'));");
        }
        $count = 0;
        $params = [
            'status' => $status,
        ...
        $url = $this->getUrl('schooldashboards/hodashboard/orders', $filters);
    ...
    public function dispatchedOrdersTabularData()
    {
		...
		FROM
		   (
		   SELECT DISTINCT so.entity_id AS order_id , so.increment_id, so.shipsy_reference_numbers, so.status, so.school_id, so.created_at FROM sales_order as so where so.status = 'dispatched_to_courier' AND so.created_at >= '$highDate' AND so.school_id = '$school_id'
		   ) AS so 
      ...
      public function rmaTableData()
    {
		...
		if (!$this->returnsData) {
			...
			$returns->addFieldToFilter('so.school_id', $school_id);
