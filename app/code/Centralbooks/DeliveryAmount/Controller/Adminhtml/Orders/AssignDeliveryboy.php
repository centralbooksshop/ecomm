...
class AssignDeliveryboy extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Centralbooks\Zones\Model\ZonesFactory $zonesF,
    ...
    public function execute()
    {
        try {
            ...
            $packageTypeValue = $this->getRequest()->getPost('packageID') ?? 0;
            ...
            foreach ($incrementIds as $key => $incrementIdsvalue) {
                ...
                if ($deliveryboyId) {
                    ...
                    $packageData      = $this->scopeConfig->getValue(self::XML_PACKAGE_TYPE, $storeScope);
                    $packageData      = explode(";", $packageData);
                    $zoneCollection   = $this->scopeConfig->getValue(self::XML_ZONE_PRICE, $storeScope);
                    $zonePrices       = explode(";", $zoneCollection);
                    $postalCode       = $order->getShippingAddress()->getPostcode();
                    $zoneData         = $this->zonesF->create()->getCollection();
                    $collection       = $zoneData->addFieldToSelect('zone')->addFieldToSelect('pincode')->addFieldToFilter('pincode', $postalCode);
                    $postalZone       = $collection->getData();
                    $deliveryAmount = 0;
                    $deliveryBoyOrder = $this->deliveryboyOrderFactory->create()->getCollection();
                    $dboCollection    = $deliveryBoyOrder->addFieldToSelect('delivery_amount')
                        ->addFieldToFilter('deliveryboy_id', $deliveryboyId)
                    ...
                    $this->logger->info("Total Amount" . print_r($totalAmount, true));

                    if (!empty($postalZone)) {

                        $postalZonePrice = (trim($postalZone[0]["zone"])) . $packageTypeValue;
                    }

                    $deliveryboyBoxesDelivered = $this->collectionFactory->create()
                        ->addFieldToFilter("deliveryboy_id", $deliveryboyId)
                        ->addFieldToFilter("order_status", 'order_delivered')
                        ->addFieldToFilter("package_type", $packageData[0])->getSize();

                    $deliveryboyCoversDelivered = $this->collectionFactory->create()
                        ->addFieldToFilter("deliveryboy_id", $deliveryboyId)
                        ->addFieldToFilter("order_status", 'order_delivered')
                        ->addFieldToFilter("package_type", $packageData[1])->getSize();

                    foreach ($zonePrices as $zonePrice) {
                        $zonePrice = trim($zonePrice);
                        if (!empty($zonePrice)) {
                            $zonePrice = explode(":", $zonePrice);
                            if (!empty($postalZone)) {
                                if ($postalZonePrice == trim($zonePrice[0])) {
                                    $deliveryAmount = trim($zonePrice[1]);
                                }
                            }
                        }
                    }
                    $totalOrders = ($deliveryboyBoxesDelivered + $deliveryboyCoversDelivered);
                    //	$this->logger->info("Covers".$deliveryboyCoversDelivered);
                    //	$this->logger->info("Boxes".$deliveryboyBoxesDelivered);
                    //	$this->logger->info("Postal Code".$postalCode);
                    //	$this->logger->info("deliveryAmount".$deliveryAmount);
                }
                ...
                if ($order->getId()) {
                    ...
                    if ($saveData) {
                        ...
                        $order->setPackageType($packageTypeValue);
             ...
             $deliveryboy_custom->setNoOfCoversDelivered($deliveryboyCoversDelivered)
                                ->setNoOfBoxesDelivered($deliveryboyBoxesDelivered)
                                ->setTotalNoOfOrders($totalOrders)
                                ->setTotalAmountToBeReceived($totalAmount)
				->save(); */
