...
/**
 * @deprecated - its better to avoid extending actions
 */
abstract class Actions extends \Magento\Backend\App\Action
{
    ...
    {
        $_preparedActions = ['index', 'grid', 'new', 'newrefund', 'editrefund', 'edit', 'save', 'delete', 'massStatus'];
     ...
     * @return void
     */
    protected function _newrefundAction()
    {
        $this->_forward('editrefund');
    }
     ...
     * @return void
     */
    public function _editrefundAction()
    {
        ...
        if ($model->getId()) {
            $breadcrumbTitle = __('Editrefund '.$this->_objectTitle);
    ...
    public function _saveAction()
    {
        ...
        try {
            $date = $this->_objectManager->get(DateTime::class)->gmtDate();
    ...
    protected function _deleteAction()
    {
        ...
        try {
            ...
            $this->messageManager->addException(
                $e,
                __('We can\'t delete '.strtolower($this->_objectTitle).' right now. '.$e->getMessage())
            );
        }
        ...
        if (!$error) {
            $this->messageManager->addSuccess(
                (count($ids) > 1)
                    ? __($this->_objectTitles.' have been deleted.')
                    : __($this->_objectTitle.' has been deleted.')
            );
        }
    ...
    protected function _massStatusAction()
    {
        ...
        try {
            ...
            $this->messageManager->addException(
                $e,
                __('We can\'t change status of '.strtolower($this->_objectTitle).' right now. '.$e->getMessage())
            );
        }
        ...
        if (!$error) {
            $this->messageManager->addSuccess(
                (count($ids) > 1)
                    ? __($this->_objectTitles.' status have been changed.')
                    : __($this->_objectTitle.' status have been changed.')
            );
        }
