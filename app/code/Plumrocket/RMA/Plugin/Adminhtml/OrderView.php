...
class OrderView
{
    ...
    public function beforeGetOrderId(View $subject)
    {
		...
		$refundurl = $this->getReturnsrefundUrl($subject);
        $subject->addButton(
            'returns_button',
            [
                'label' => __('Return'),
     ...
     * @return string
     */
    public function getReturnsrefundUrl($subject)
    {
        // Can't call method $subject->getOrderId(), because this plugin use it.
        $orderId = $subject->getOrder()->getId();
        return $this->urlBuilder->getUrl('prrma/returns/newrefund', [
            'order_id' => $orderId
        ]);
    }
