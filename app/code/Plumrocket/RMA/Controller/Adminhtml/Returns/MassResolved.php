...
class MassResolved extends Action
{
    protected $filter;
    ...
    public function execute()
    {
          try {
            ...
            foreach ($collection as $item) {
                $returnModel = $this->returnsModelFactory->create()->load($item['entity_id']);
				...
				$sql = "Update " . $tableName . " Set status = 'processed_closed' where entity_id = " .$item['entity_id'];
				...
				$itemCollection = $this->itemFactory->create()->getCollection()
                                ...
                                foreach ($itemCollection as $collectionData) {
                                        ...
                                        if($qtyRequested > 0 && $qtyReceived > 0){
						$returnItemData->setQtyApproved($qtyRequested)->save();
						$returnModel->setIsClosed(true)->save();
