...
class MassApproved extends Action
{
    protected $filter;
    ...
    public function execute()
    {
          try {
			...
			foreach ($collection as $item) { 
				...
				$sql = "Update " . $tableName . " Set status = 'authorized' where entity_id = " .$item['entity_id'];
				...
				//	$this->logger->info('data  out - '.$rmaStatus." ".$qtyRequested);

					if($qtyRequested > 0){
					    $this->logger->info('data - '.$item['status'] ." ".$qtyRequested);
