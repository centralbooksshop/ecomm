...
use Plumrocket\RMA\Model\Reasonlayer;
use Plumrocket\RMA\Model\Reasonreplace;
use Plumrocket\RMA\Model\Reasoncancel;
...
class Itemsrefund extends Template
{
    /**
     * Image size
     */
     ...
     * @var Reasonlayer
     */
    protected $Reasonlayer;

    /**
     * @var Reasonreplace
     */
    protected $Reasonreplace;
    ...
    protected $info = [
        ...
        ItemHelper::REASON_ID_LAYER     => '"Return Reason" is why customers are returning an item. You can edit or add new "Return Reasons" in Plumrocket RMA -> Return Reasons.',
        ItemHelper::REASON_ID_REPLACE     => '"Return Reason" is why customers are returning an item. You can edit or add new "Return Reasons" in Plumrocket RMA -> Return Reasons.',
        ItemHelper::REASON_ID_CANCEL     => '"Return Reason" is why customers are returning an item. You can edit or add new "Return Reasons" in Plumrocket RMA -> Return Reasons.',
     ...
     * @param Reason              $reason
     * @param Reasonlayer         $reasonlayer
     * @param Reasonreplace       $reasonreplace
    ...
    public function __construct(
        ...
        $this->reasonreplace = $reasonreplace;
    ...
    public function getReasonOptionsLayer()
    {
        $collection = $this->reasonlayer
            ->getCollection()
            ->addActiveFilter()
    ...
    public function getReasonOptionsReplace()
    {
        $collection = $this->reasonreplace
            ->getCollection()
            ->addActiveFilter()
            ->addStoreFilter($this->getOrder()->getStoreId())
            ->setOrder('position', 'asc');
    ...
    public function getReasonOptionsCancel()
    {
        $collection = $this->reasoncancel
            ->getCollection()
            ->addActiveFilter()
            ->addStoreFilter($this->getOrder()->getStoreId())
            ->setOrder('position', 'asc');
    ...
    public function getColumns()
    {
        $columns = [
            ...
            ItemHelper::REASON_ID_LAYER       => $this->itemHelper->getCols(ItemHelper::REASON_ID_LAYER),
            ItemHelper::REASON_ID_REPLACE       => $this->itemHelper->getCols(ItemHelper::REASON_ID_REPLACE),
            ItemHelper::REASON_ID_CANCEL       => $this->itemHelper->getCols(ItemHelper::REASON_ID_CANCEL),
    ...
    public function getElements(Item $item, $name = null)
    {
        ...
        if (empty($this->itemElements[$rowKey])) {
            ...
            if(is_numeric($rowKey) == 1){
                $connection = $this->resourceConnection->getConnection();
                $table = $connection->getTableName('plumrocket_rma_returns_item');
                $query = "SELECT reason_id_layer,reason_id_replace,reason_id_cancel FROM `" . $table . "` WHERE entity_id = $rowKey ";
                $result1 = $connection->fetchAll($query);

                if($result1[0]['reason_id_layer']!=''){
                    // Return Reason layer.
                    $key = ItemHelper::REASON_ID_LAYER;
                    $elements[$key] = $this->createElement($key, 'select', [
                        'name'      => "items[$rowKey][$key]",
                        'label'     => $this->itemHelper->getCols($key),
                        'options'   => $this->getReasonOptionsLayer(),
                    ], $form);
                }
                
                if($result1[0]['reason_id_replace']!=''){
                    $key = ItemHelper::REASON_ID_REPLACE;
                    $elements[$key] = $this->createElement($key, 'select', [
                        'name'      => "items[$rowKey][$key]",
                        'label'     => $this->itemHelper->getCols($key),
                        'options'   => $this->getReasonOptionsReplace(),
                    ], $form);
                }
                if($result1[0]['reason_id_cancel']!=''){
                    $key = ItemHelper::REASON_ID_CANCEL;
                    $elements[$key] = $this->createElement($key, 'hidden', [
                        'name'      => "items[$rowKey][$key]",
                        'label'     => $this->itemHelper->getCols($key),
                        //'options'   => $this->getReasonOptionsCancel(),
					      'value'     => 3,
                    ], $form);
                }
            ...
            $elements[$key] = $this->createElement($key, 'text', [
                'name'      => "items[$rowKey][$key]",
                'label'     => $this->itemHelper->getCols($key),
               // 'min'       => '0',
                'value'       => $qtyMax,
