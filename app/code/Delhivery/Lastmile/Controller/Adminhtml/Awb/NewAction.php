...
class NewAction extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
		...
		if($apiurl && $token)
		{
			...
			if(count($userModel) > 1000){
				$this->messageManager->addErrorMessage(__('More than 1000 AWB still available to use.'));
				$resultRedirect = $this->resultRedirectFactory->create();
				$resultRedirect->setPath('delhivery_lastmile/awb');
				return $resultRedirect;
			}
			
			$path = $apiurl.'json/?token='.$token.'&count=5000&cl='.urlencode($cl);
