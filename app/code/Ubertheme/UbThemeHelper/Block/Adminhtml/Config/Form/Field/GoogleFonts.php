...
class GoogleFonts extends \Ubertheme\UbThemeHelper\Block\Adminhtml\Config\Form\Field
{
    const CACHE_LIFETIME_FONTS = 86400;
    const CACHE_IDENTIFIER_FONTS = 'ub_google_fonts';
    const URL_GET_FONTS =  'https://google-webfonts-helper.herokuapp.com/api/fonts';
                ...
                "jquery",
                 ...
                 "webfont",
                    ...
                    $(document).ready(function () {
                        $( "#' . $element->getHtmlId() . '" ).css("width", "230px");

                        var previewFontStyle = function(font) {
                            $("#' . $element->getHtmlId() . '").siblings("p.note").css("font-family", font).css("font-size", "18px");
                        }

                        //load for selected font
                        WebFont.load({
                            google: {
                                families: ["' . $value . '"]
                            },
                        ...
                        $.fn.selectGoogleFont = function(options) {
                            var settings = $.extend({
                            ...
                            self.select2({
                                ...
                                /*templateResult: function (result) {
                                    ...
                                    //var state = $(\'<div style = "font-family: \' + result.text + \' !important;" class="li_\'+result.itemId+\'" > \'+result.text+\'</div>\');
                                    y	+=29;
                                    return state;
                                }*/
                        ...
                        $( "#' . $element->getHtmlId() . '" ).selectGoogleFont({
                            selectedCallback:function(font){
                                $( "#' . $element->getHtmlId() . '" ).val(font);
                            },
                            loadedCallback:function(font){
                                previewFontStyle(font);
                            }
                        });

                        $( "#' . $element->getHtmlId() . '" ).on("change", function() {
                            previewFontStyle($(this).val());
                        });
                    });
                });
            </script>';
    ...
    {
        $fonts = [];
        $cacheId = md5(self::CACHE_IDENTIFIER_FONTS);
        $data = $this->cache->load($cacheId);
        if (!$data) {
            $url = self::URL_GET_FONTS;
            $data = $this->getContent($url);
            //save to cache
            $this->cache->save($data, $cacheId, [], self::CACHE_LIFETIME_FONTS);
        }
    ...
    /*
    protected function numToText($num) {
        static $maps = array(
            100 => 'Thin',
            200 => 'Extra Light',
            300 => 'Light',
        ...
        return isset($maps[$num]) ? $maps[$num] : 'Regular';
    }

    protected function getGFonts()
    {
        $fontNames = [
    ...
    {
        //$url = 'https://github.com/honeysilvas/google-fonts/blob/dev/csv/google-web-font-list.csv';
        $url = 'https://github.com/honeysilvas/google-fonts/blob/dev/json/google-web-font-list-sorted-by-alphabetical-order.json';
        ...
        foreach ($lines as $line) {
            ...
            if (count($tmp) > 2) {
                ...
                $ftype = $tmp[1];
                $fweight = $tmp[2];
                $tmp[2] = $this->numToText($tmp[2]);
                $tmp[1] = ucfirst($tmp[1]);
            } else continue;

            if (!in_array($fname, $fontNames)) {
                $fontNames[] = $fname;
            }
            //$font['name'] = $fname;
            //$font['type'] = $ftype;
            //$font['weight'] = $fweight;
            //$font['title'] = implode (' ', $tmp);
            //$font['category'] = $arr[4];
            //$font['value'] = $f;
            //$fonts[] = $font;
        }
