...
class Mega extends AbstractHelper
{
    ...
    public function rebuildData($items)
    {
        ...
        foreach ($items as $item) {
             ...
             * col3=50
             */
			if(!empty($item->getMegaColXWidth())) {
            ...
            if (in_array($this->deviceType, $megaParam->visible_in)) {
                ...
                if (is_array($blocks) AND count($blocks) > 0) {
                    ...
                    for ($col = 0; $col < $cols; $col++) {
                        ...
                        if ($cols > 1)
                            $content .= $this->beginSubMenuContent($item, $col, $pos, 1, true);
                        $i = $col;
                        while ($i < $total) {
                            $block = $blocks[$i];
                            $i += $cols;
    ...
    public function genMenuItems($pid, $level)
    {
        if (isset($this->children[$pid]) AND $this->children[$pid]) {
            ...
            for ($col = 0, $j = 0; $col < $cols AND $j < $total; $col++) {
                ...
                $this->beginSubMenuItems($col, $pos, $level, $pid);
...
//                    }
    ...
    public function beginSubMenuContent($item, $i, $pos, $level = 0, $return = false)
    {
        $data = '';
    ...
    public function beginSubMenuItems($i, $pos, $level = 0, $pid = 0, $return = false)
    {
        $level = (int)$level;
        $data = '';
    ...
    public function genMenuItem($item, $level = 0, $pos = '', $return = 0)
    {
        ...
        $titleAttr = ($menuTitle) ? strip_tags("title=\"{$menuTitle}\"") : "";
        ...
        if ($menuText){
            if ($item->getLinkType() != \Ubertheme\UbMegaMenu\Model\Item::LINK_TYPE_NO_LINK) {
                //add target to tag a
                switch ($item->getLinkTarget()) {
                    default:
                    case '_top':
                        $data .= '<a href="' . $link . '" ' . $class . ' ' . $titleAttr . '>' . $menuText . '</a>';
                        break;
                    case '_blank':
                        $data .= '<a href="' . $link . '" target="_blank" ' . $class . ' ' . $titleAttr . '>' . $menuText . '</a>';
                        break;
                    case '_parent':
                        $data .= '<a href="' . $link . '" target="_parent" ' . $class . ' ' . $titleAttr . '>' . $menuText . '</a>';
                        break;
                }
            } else {
        ...
        if (isset($megaParam->desc) AND $megaParam->desc) {
            $desc = \Magento\Framework\App\ObjectManager::getInstance()->create('Ubertheme\Base\Model\TemplateFilter')->filter($megaParam->desc);
    ...
    public function loadBlocks($item)
    {
        ...
        switch ($subContentType) {
            case 'static-block':
                ...
                return $blocks;
            case 'custom-content':
                $block = \Magento\Framework\App\ObjectManager::getInstance()->create('Ubertheme\Base\Model\TemplateFilter')->filter($item->getCustomContent());
                return $block;
            default:
                return null;
        }
