...
class OrderDetails extends Column
{
    ...
    {
        $this->logger->info('prepareDataSource start ');
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
				...
				//$this->logger->info('prepareDataSource end ');
    ...
    {
        $this->logger->info('prepareDataSource start ');
        $this->logger->info('deliveryMethods start ');       
        ...
        $this->logger->info('deliveryMethods end '); 
		//echo '<pre>'; print_r($shipping_methods);die; 
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                $this->logger->info('dataSource start ' . $item['entity_id']);
				...
				$slip = '';
				$Cancel = '';
                $this->logger->info('shipments start '); 
                ...
                $this->logger->info('shipments end ');
                ...
                try {
				$order_entity_id = $this->_orderRepository->get((int)$item['entity_id']);
				} catch (\Magento\Framework\Exception\NoSuchEntityException $e) {
					$this->logger->warning(
						'Order not found for entity_id: ' . $item['entity_id']
					);
					continue;
				}


                $totalQty = $order_entity_id->getTotalItemCount(); 
                $orderQty = $this->getOrderedQty($order_entity_id);

                /*foreach ($order_entity_id->getAllVisibleItems() as $itemNew) {
                    $name= $itemNew->getName();
                    $item['product_name'] = $name;
                }*/


                $invoicesdCount = $this->getInvoicedCount($order_entity_id);
                $status = $order_entity_id->getData("gift_message_id");
			...
			$this->logger->info('dataSource end ' . $item['entity_id']);
            }
        }
        $this->logger->info('prepareDataSource end ');
        return $dataSource;
