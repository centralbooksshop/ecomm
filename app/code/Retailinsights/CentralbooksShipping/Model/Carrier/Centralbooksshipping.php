 ...
 */
class Centralbooksshipping extends AbstractCarrier implements CarrierInterface
{
    const WEBSITE_CODE_BASE = 'base';
    const WEBSITE_CODE_SCHOOL = 'school';
    protected $_code = 'centralbooksshipping';
	...
	if($hybridDeliveryStatus == 1){
		...
		if ($request->getFreeShipping() === true) {
	        	...
	        	if($currentWebsiteCode == 'schools'){
					...
					$hybrid_product_shipping_fee = $rateData['hybrid_product_shipping_fee'];
					$this->logger->info(" rateData : ". print_r($rateData, true));
        	        	}
	                }else {
                ...
                $methodShipping = $this->rateMethodFactory->create();
                $methodShipping->setCarrier($this->_code);
                $methodShipping->setCarrierTitle('Home Delivery');
                $method_shipping_title = $this->getConfigData('name').' (Incl. GST)';
                $methodShipping->setMethod($this->_code);
		$methodShipping->setMethodTitle($method_shipping_title);
		if($hybrid_product_shipping_fee >= 0){
			$methodShipping->setPrice($hybrid_product_shipping_fee);
		}else{
			$methodShipping->setPrice($finalPrice);
		}
		$methodShipping->setCost($shippingBaseCost);
		$result->append($methodShipping);
	}else{
	    ...
	    if ($request->getFreeShipping() === true) {
            ...
            if($currentWebsiteCode == 'schools'){
                ...
                if(isset($shipping_amt)) {
                   ...
                   $finalPrice = $this->getAdditionalRateForSchoolWebsite();
                }
	    }else {
	   ...
	   $method = $this->rateMethodFactory->create();
           ...
           if($product_handling_fee > 0 && $product_handling_fee != '') {
            $method->setCarrierTitle($this->getConfigData('handlingtitle'));
    	    $method_title = $this->getConfigData('handlingname');
	   } else {
	    $method->setCarrierTitle($this->getConfigData('title'));
	    $method_title = $this->getConfigData('name').' (Incl. GST)';
	   } 
           $method->setMethod($this->_code);
	   $method->setMethodTitle($method_title); 
	   $method->setPrice($finalPrice);
           $method->setCost($shippingBaseCost);
