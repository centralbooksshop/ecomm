...
{	protected $postCollection;
     ...
     *
     */
     const EXPORT_ORDER_START_DATE = 'orderexport/orderexportdate/startdate';
    /**
    ...
    public function execute() {
		...
		$fromDate_val = $this->scopeConfig->getValue(self::EXPORT_ORDER_START_DATE, $storeScope);
        $toDate_val = $this->timezone->date()->format('Y-m-d');
		...
		try {
			if (strtotime($fromDate_val) > strtotime($toDate_val)) 
			{
				...
				$this->_redirect('orderexport/exportorder/download');
				return;
			}
			$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
			$fileSystem = $objectManager->create('\Magento\Framework\Filesystem');
			$mediaPath = $fileSystem->getDirectoryRead(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA)->getAbsolutePath();
			$finalmediaPath = $mediaPath.'orderexport.csv';
				...
				if ($orderCollection && count($orderCollection) > 0) {
					...
					if($schoolCode!= ''){
					    ...
					    if($item->getProductType()!="bundle") {
							...
							$totalwithoutDiscount=$item->getQtyOrdered()*$item->getPrice();
