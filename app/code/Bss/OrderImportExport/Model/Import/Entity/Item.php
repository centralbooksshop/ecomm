...
class Item extends AbstractEntity
{
    ...
    public function extractRowData(array $rowData, $rowNumber = 0)
    {
		...
		$item_product_type = $rowData['item:product_type'];
		//echo 'extractRowData<pre>';print_r($rowData);die;
		//if(!empty($item_product_type) && $item_product_type == 'simple' ) {}
        $item_product_options = $rowData['item:product_options'];
		if(!empty($item_product_type) && $item_product_type == 'bundle' ) {
            $final_item_product_options ='{"info_buyRequest":{"uenc":"aHR0cDovLzEyNy4wLjAuMS9tYWdlbnRvMjM0L2RlZmF1bHQvZ2Vhci9iYWdzLmh0bWw","product":"'.$item_product_options.'","selected_configurable_option":"","related_product":"","item":"'.$item_product_options.'","bundle_option":{},"qty":"1","bundle_option_qty":{}},"bundle_options":{},"product_calculations":0,"shipment_type":"0"}';
			$rowData = array_replace($rowData, ['item:product_options' => $final_item_product_options]); 
		}
	...
	public function extractRowDatamain(array $rowData, $rowNumber = 0)
    {
        $rowData = parent::extractRowData($rowData);
        $rowData = $this->extractFields($rowData, $this->prefixCode);
