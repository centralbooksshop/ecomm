...
class ShipmentSaveActionAfter implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->_coreRegistry->registry('fedexlabel_order_id') !== null) {
            ...
            if ($paramShipment !== null && isset($paramShipment['infomodus_fedex_label']) && $paramShipment['infomodus_fedex_label'] == 1) {
                return $this->_context->getResponse()
                    ->setRedirect($this->_context->getUrl()->getUrl('infomodus_fedexlabel/items/edit',
                        ['order_id' => $order_id, 'shipment_id' => $shipment_id, 'direction' => 'shipment', 'redirect_path' => 'shipment']))->sendResponse();
            }
        }
