...
class Payu extends AbstractHelper
{
    ...
    public function createTempOrder($txnId)
    {
        ...
        if ($m2order->getEntityId()) {
            $result['increment_id'] = $m2order->getRealOrderId();
            $result['order_id'] = $m2order->getId();
        } else {
    ...
    public function setDiscount($order, $customDiscount, $discountDescription)
    {
        ...
        $order->save();
        // $order->setGrandTotal($order->getBaseSubtotal()-$customDiscount)
        // ->setBaseGrandTotal($order->getBaseSubtotal()-$customDiscount)
        // ->save();
