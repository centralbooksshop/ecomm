...
class OrderUpdate
{
    ...
    )
    {
        ...
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/payuordercron.log');
    ...
    {
        $this->logger->info("PayU cron is started ".date("Y-m-d H:i:s"));
       ...
       // $this->logger->info(json_encode($orderList->getData()));
        $this->logger->info($orderList->getSelect()->__toString());
        $this->logger->info("PayU no of orders processed is  ".count($orderList));
        foreach ($orderList as $orderData){
            $order=$this->orderModel->load($orderData->getId());
            $txnid=$orderData->getQuoteId();
        ...
        $this->logger->info('PayU Cron END AT ' . $endAt->format('Y-m-d H:i:s') . ' duration_sec: ' . round($endMicro - $startMicro, 3).
