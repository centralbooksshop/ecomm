...
class Payu extends \Magento\Payment\Model\Method\AbstractMethod
{
	...
	public function buildCheckoutRequest()
	{
		...
		$create_order_id = $this->helper->createTempOrder($txnid);
		$amount = round($quote->getBaseGrandTotal(), 2);
		$udf4 		= $create_order_id['increment_id'];
		$udf5 		= $create_order_id['order_id'];
	...
	public function generatePayuHash(
		...
		//$deb['string']="Hash String-".$hash_string;
		//$deb['hash']="Hash-".strtolower(hash('sha512', $hash_string));

		//$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/payu.log');
		//$logger = new \Zend_Log();
		//$logger->addWriter($writer);
		//$logger->info($deb['string']);
		//$logger->info($deb['hash']);
