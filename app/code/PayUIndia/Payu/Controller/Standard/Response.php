...
class Response extends \PayUIndia\Payu\Controller\PayuAbstract implements CsrfAwareActionInterface, HttpGetActionInterface,HttpPostActionInterface
{
    ...
    public function execute() {
        ...
        try {
			...
			//echo " response <pre>";print_r($params);die;

			$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/payu.log');
			$logger = new \Zend_Log();
			$logger->addWriter($writer);
			$logger->info('payu response start');
			//$logger->info('payu response');
			...
			if ($paymentMethod->validateResponse($params)) {
				...
				if(empty($order) === false)
				{
					...
					$existingTxn = $order->getPayment()->getLastTransId();

					if (!empty($existingTxn)) {

						$logger->info("Duplicate PayU response ignored for Order: ".$order->getIncrementId());
						...
						return $this->getResponse()->setRedirect(
							$this->getCheckoutHelper()->getUrl('checkout/onepage/success')
						);
					}
				...
				//$order = $this->getOrder();	
				//$this->_logger->debug("To Process Order: ".$order->getIncrementId());
			...
			echo $e->getMessage();
			// print_R($e->getTrace());die(__FILE__);
			die();
		...
		$logger->info('payu response end');
		//$this->_logger->debug("Return URL: ".$returnUrl);
