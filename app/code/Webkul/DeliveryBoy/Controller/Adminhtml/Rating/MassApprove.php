...
class MassApprove extends \Webkul\DeliveryBoy\Controller\Adminhtml\Rating
{
    /**
     * Mass approve Ratings.
     *
    ...
    public function execute()
    {
        ...
        foreach ($collection->getAllIds() as $ratingId) {
            ...
            if (count($ratingData)) {
                ...
                $ratingAfterSave = $this->ratingDataFactory->create()->load($ratingId)
                    ->setStatus(Rating::STATUS_APPROVED);
        ...
        $collection->setRatingData($coditionData, ["status"=>1]);
        if ($ratingUpdated) {
            $this->messageManager->addSuccess(__("A total of %1 rating(s) were approved.", $ratingUpdated));
        }
    ...
    /**
     * Dispatch Event for Evaluating Rating.
     *
