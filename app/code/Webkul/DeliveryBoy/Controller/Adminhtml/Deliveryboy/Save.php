...
class Save extends \Webkul\DeliveryBoy\Controller\Adminhtml\Deliveryboy
{
	...
	$deliveryboyId = $originalRequestData["expressdelivery_deliveryboy"]["id"] ?? null;
        $isNewDeliveryboy = empty($originalRequestData["expressdelivery_deliveryboy"]["id"])
            ? true : false;
        if ($originalRequestData) {
            try {
		    $deliveryboyData = $originalRequestData["expressdelivery_deliveryboy"];
		    //echo $deliveryboyData["driver_type"];
		 $isExisting = (bool)$deliveryboyId;
                ...
                if ($deliveryboyData["password"] != "" && $deliveryboyData["confpassword"] != "") {
                    if ($deliveryboyData["password"] == $deliveryboyData["confpassword"]) {
                        $deliveryboyData["password"] = $this->operationHelper
                            ->getSha256Hash($deliveryboyData["password"]);
                    } else {
                ...
                if ($isNewDeliveryboy !== true) {
                    $deliveryboy->load($deliveryboyId);
                    $oldApproveStatus = $deliveryboy->getApproveStatus();
                }
                ...
                if ($isNewDeliveryboy) {
                    $this->deliveryboyHelper->sendDeliveryboyRegistrationEmail($deliveryboyId);
                }
                $newApproveStatus = $originalRequestData["expressdelivery_deliveryboy"]["approve_status"];
                if (($isNewDeliveryboy && $newApproveStatus == ApproveStatus::STATUS_APPROVED) ||
                    (isset($oldApproveStatus) && $oldApproveStatus != ApproveStatus::STATUS_APPROVED
                    && $newApproveStatus == ApproveStatus::STATUS_APPROVED)
                ) {
                    // Send email to deliveryboy.
                    $deliveryboyId;
		}
		...
		$driverData = array(
					'no' => $deliveryboy->getVendorId(),
                                        'deliveryBoyWebID' => $deliveryboy->getId(),
                                        'name' => $deliveryboy->getName(),
                                        'email' => $deliveryboy->getEmail(),
                                        'mobilePhoneNo' => $deliveryboy->getMobileNumber(),
                                        'address' => $deliveryboy->getAddress(),
                                        'vehicleNo'=> $deliveryboy->getVehicleNumber(),
		);
		$vendorUpdatePayload = array(
                                        'deliveryBoyWebID' => $deliveryboy->getId(),
                                        'name' => $deliveryboy->getName(),
                                        'email' => $deliveryboy->getEmail(),
                                        'mobilePhoneNo' => $deliveryboy->getMobileNumber(),
		...
		$driverData = json_encode($driverData);
		$vendorUpdatePayload = json_encode($vendorUpdatePayload);
		...
		$api_url_key = "apiUpdateDeliveryBoy";
		if($driverType == "Parent"){
			$erpResponse = $this->apiDataCustom->apiCall($api_url_key, $erpToken, $driverData, $headersGP, $vendorUpdatePayload);
	//		print_r($erpResponse);die;
		if(!empty($erpResponse['id'])){
			 $deliveryboy->setDriverErpId($erpResponse['id'])->save();
		}
		}
