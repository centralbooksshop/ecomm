...
{
    /**
     * Validate Deliveyboy Image type.
     *
    ...
    protected function _validateDeliveryboy($response)
    {
	    ...
	    $this->logger->info("DID : ".$dataResult["vendor_id"]);
	    $vendorIdCheck = $this->collectionFactory->create()
                ->addFieldToFilter("vendor_id", $dataResult["vendor_id"])
		->getFirstItem();
	    ...
	    }elseif ((bool)$vendorIdCheck->getId()
                && $vendorIdCheck->getId() != $deliveryboyId
            ) {
                $errors[] = __("Delivery boy with same Vendor ID already exist.");
    ...
    /**
     * Pack and return validation result in Json.
     *
