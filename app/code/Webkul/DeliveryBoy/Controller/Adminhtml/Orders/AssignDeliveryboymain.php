...
class AssignDeliveryboymain extends \Magento\Backend\App\Action
{
    /**
     * Current order otp
     *
    ...
    public function execute()
    {
        ...
        try {
				...
				if(!empty($deliveryboyorder_limit)) {
				  //$order_limit_diff = $deliveryboyorder_limit - $deliverboyordercoll;
					if ($deliveryboyorder_limit <= $deliverboyordercoll) {
						return $result->setData($this->getFailureResultArray((string)__("Deliveryboy Order limit exceeded")));
					}
				}
				// print_r(count($deliveryboyOrderColl));die;
			   }
			...
			//ravi custom code start
			    if($order->getId()){
					...
					if($saveData){
						...
						$order->save(); 
						$this->SendMessageAssignDriver($deliveryboyId, $order);
					} 
				} 
            ...
            return $result->setData($this->getSuccessResultArray());
        } catch (\Exception $e) {
	...
	public function SendMessageAssignDriver($deliveryboyId, $order): void
	   {
        ...
        $msg = "Dear ".$custmerName.", Your order ".$incrementId." has been successfully assigned to the Delivery Boy ".$deliveryboyName.". You will receive the order soon, your OTP for delivery is ".$this->getOtp().". Reach us at ".$helpdesklink." for any assistance - centralbooksonline.com.";
     ...
     * @return void
     */
    public function sendEmail($deliveryboyId, $order): void
    {
        try {
            $deliveryboy = $this->deliveryboy->load($deliveryboyId);
            $deliveryboyName = $deliveryboy->getName();
