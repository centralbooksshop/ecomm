...
class SetStatusOrderPickedupAll extends \Webkul\DeliveryBoy\Controller\Api\AbstractDeliveryboy
{
    /**
     * Set Status order picked up.
     *
    ...
    public function execute()
    {
        try {
            ...
            $this->returnArray["message"] = __("Orders picked up from store.");
    ...
    private function orderPickup()
    {
        ...
        $orderState = 'dispatched_to_courier';
        ...
        $deliveryboyOrder->setPicked(1)->save();
        ...
        $this->sendEmail($deliveryboyOrder->getOtp());
    ...
    public function sendNotificationToCustomer(): void
    {
		...
		if(!empty($customer_id)) {
			...
			$tokenCollection = $this->tokenFactory
				->create()
				->getCollection()
				->addFieldToFilter("deliveryboy_id", $customer_id);
			$this->sendNotification($fields, $message, $tokenCollection);
		}
    }

    /**
     * Send Notification to delvieryboy.
     *
     * @return void
     */
     ...
     * @return void
     */
    public function sendEmail($otp): void
    {
        try {
            $templateVariables = [];
