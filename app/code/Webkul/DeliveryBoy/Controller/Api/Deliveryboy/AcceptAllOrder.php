...
class AcceptAllOrder extends \Webkul\DeliveryBoy\Controller\Api\AbstractDeliveryboy
{
    /**
     * Accept All Order.
     *
    ...
    public function execute()
    {
        try {
            ...
            $incrementIds = explode(',', $this->incrementId);
            foreach($incrementIds as $inc) {
                $this->incrementId = $inc;
                $this->orderAccept();
            }
    ...
    private function orderAccept()
    {
        ...
        $this->_eventManager->dispatch(
            'wk_deliveryboy_accept_order_before_accept_event',
            [
                'deliveryboy_order' => $deliveryboyOrder,
                'assign_status' => $this->assignStatus,
            ]
        );
        if ($deliveryboyOrderId) {
            ...
            $this->returnArray["message"] = (string)__("Request processed successfully.");
            $this->returnArray["success"] = true;
        } else {
            $this->returnArray["message"] = (string)__("Invalid delivery boy order.");
        }
    }
     ...
     * @return void
     */
    public function sendNotificationToAdmin(): void
    {
    ...
    /**
     * Send Notification to deliveyboy.
     *
     * @return void
     */
    public function sendNotificationToDeliveryboy(): void
    {
        $authKey = $this->deliveryboyHelper->getFcmApiKey();
        if (empty($authKey)) {
            return ;
        ...
        if ($this->assignStatus == '1') {
            $message += [
                "body" => __("You have successfully accepted the Delivery for order #%1", $this->incrementId),
                "title" => __("Delivery Accepted #%1", $this->incrementId),
                "message" => __("You have successfully accepted the Delivery for order #%1", $this->incrementId),
            ];
        } else {
            $message += [
