...
use Webkul\DeliveryBoy\Helper\DeliveryAutomation as DeliveryAutomationHelper;
...
abstract class AbstractRating extends \Webkul\DeliveryBoy\Controller\Api\AbstractDeliveryboy
{
     ...
     * @param DeliveryAutomationHelper $deliveryAutomationHelper
     */
    public function __construct(
        ...
        DeliveryAutomationHelper $deliveryAutomationHelper
    ) {
            parent::__construct(
                $emulate,
                $config,
    ...
    public function buildReviewDataForAppFromReviewModel($mFactor, $reviewModel)
    {
        ...
        $customerModel = $this->deliveryboyHelper->getCustomerModelById(
            $reviewModel->getCustomerId()
        );
        return [
            "id" => $reviewModel->getId(),
            "deliveryboyId" => $reviewModel->getDeliveryboyId(),
            "name" => $deliveryboyModel->getName(),
            "avatar" => $this->helperCatalog->resizeAndGetDeliveryboyImageFromMFactorAndImageRelativePath(
                $mFactor,
                $deliveryboyModel->getImage()
            ),
            "title" => $reviewModel->getTitle(),
            "comment" => $reviewModel->getComment(),
            "customerId" => $reviewModel->getCustomerId(),
            "rating" => $reviewModel->getRating(),
            "status" => $reviewModel->getStatus(),
            "statusLabel" => $this->deliveryboyHelper->getReviewStatusLabelByReviewStatus(
                $reviewModel->getStatus()
            ),
     ...
     * @return array
     */
    public function getDeliveryboyReviewStatusList()
    {
        return $this->ratingStatus->toOptionArray();
    }
}
